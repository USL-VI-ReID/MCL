...
def extract_cnn_feature(model, inputs,mode,cid):
    inputs = to_torch(inputs).cuda()
    if mode == 1:
        outputs = model(inputs,inputs,modal=mode,cid_rgb=cid)
    else:
        outputs = model(inputs,inputs,modal=mode,cid_ir=cid)
...
def extract_features(model, data_loader, print_freq=50,flip=True,mode=0):
    ...
    with torch.no_grad():
        for i, (imgs, fnames, pids, cid, _) in enumerate(data_loader):
            ...
            outputs = extract_cnn_feature(model, imgs,mode,cid)
            flip = fliplr(imgs)
            outputs_flip = extract_cnn_feature(model, flip,mode,cid)
