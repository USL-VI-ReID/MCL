...
class build_transformer(nn.Module):
    def __init__(self, num_classes, camera_num, view_num, cfg, factory):
        ...
        self.bottleneck_rgb = nn.BatchNorm1d(self.in_planes)
        self.bottleneck_rgb.apply(weights_init_kaiming)
        
        self.bottleneck_ir = nn.BatchNorm1d(self.in_planes)
    ...
    def forward(self, x1, x2, modal=0,label_1=None,label_2=None,cid_rgb=None,cid_ir=None,index_rgb=None,index_ir=None):
        ...
        features,feat1,feat2,label_1,label_2 = self.base(x1,x2,modal=modal,label_1=label_1,label_2=label_2,cid_rgb=cid_rgb,cid_ir=cid_ir)
        ...
        if self.training:
                rgb_feat = self.bottleneck_rgb(global_feat[:single_size_1])
                ir_feat = self.bottleneck_ir(global_feat[single_size_1:])
                self.bottleneck_ir.eval()
                rgb2ir_feat = self.bottleneck_ir(global_feat[:single_size_1])
                self.bottleneck_ir.train()
                self.bottleneck_rgb.eval()
                ir2rgb_feat = self.bottleneck_rgb(global_feat[single_size_1:])
                self.bottleneck_rgb.train()
                return feat,feat[:single_size_1],feat[single_size_1:],feat[:single_size_1],feat[single_size_1:],label_1,label_2,cid_rgb,cid_ir,index_rgb,index_ir,rgb_feat,rgb2ir_feat,ir_feat,ir2rgb_feat
        else:
            if modal == 0:
                rgb2ir_feat = self.bottleneck_ir(global_feat[:single_size_1])
                ir2rgb_feat = self.bottleneck_rgb(global_feat[single_size_1:])
            else:
                rgb2ir_feat = self.bottleneck_ir(global_feat)
                ir2rgb_feat = self.bottleneck_rgb(global_feat)
            return feat, rgb2ir_feat, ir2rgb_feat
