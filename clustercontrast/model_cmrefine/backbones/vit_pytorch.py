...
class TransReID(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0., camera=0, view=0,drop_path_rate=0., norm_layer=partial(nn.LayerNorm, eps=1e-6), local_feature=False, sie_xishu =1.0, hw_ratio=1, gem_pool = False, stem_conv=False):
        ...
        # self.sie_embed_m = nn.Parameter(torch.zeros(2, 1, embed_dim))
    ...
    def forward(self, x1_input, x2_input,modal=0,label_1=None,label_2=None,cid_rgb=None,cid_ir=None):#,gallery_1=None,gallery_2=None):#(self, x, camera_id, view_id):
        # B = x.shape[0]
        ...
        if modal == 0:
            ...
            # m_id_rgb=torch.ones_like(cid_rgb)
            # m_id_ir=torch.zeros_like(cid_ir)
            # sie_cam = torch.cat((self.sie_embed_rgb[cid_rgb],self.sie_embed_ir[cid_ir]),0 )
            # sie_m = torch.cat((self.sie_embed_m[m_id_rgb],self.sie_embed_m[m_id_ir]),0 )
        elif modal == 1:

            x = self.patch_embed(x1_input)
            # m_id_rgb = torch.ones_like(cid_rgb)
            # sie_m = self.sie_embed_m[m_id_rgb]
            # sie_cam = self.sie_embed_rgb[cid_rgb]
        elif modal == 2:
            x = self.patch_embed2(x2_input)
            # sie_cam = self.sie_embed_ir[cid_ir]
            # m_id_ir=torch.zeros_like(cid_ir)
            # sie_m = self.sie_embed_m[m_id_ir]
        ...
        x = x + self.pos_embed# + 3*(sie_m+sie_cam)
        x = self.pos_drop(x)
