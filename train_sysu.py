...
from clustercontrast.trainers_s123 import \
    ClusterContrastTrainer_pretrain_camera_confusionrefine
from clustercontrast.trainers_s123 import \
    ClusterContrastTrainer_pretrain_joint
from clustercontrast.trainers_s123 import \
    ClusterContrastTrainer_pretrain_camera_wise_3_cmrefine
...
def extract_gall_feat(model, gall_loader, ngall):
    ...
    with torch.no_grad():
        for batch_idx, (input, label) in enumerate(gall_loader):
            ...
            feat_fc, _, _ = net(input, input, 1)
            flip_input = Variable(flip_input.cuda())
            feat_fc_1, _, _ = net(flip_input, flip_input, 1)
...
def extract_query_feat(model, query_loader, nquery):
    ...
    with torch.no_grad():
        for batch_idx, (input, label) in enumerate(query_loader):
            ...
            flip_input = fliplr(input)
            input = Variable(input.cuda())
            feat_fc, _, _ = net(input, input, 2)
            flip_input = Variable(flip_input.cuda())
            feat_fc_1, _, _ = net(flip_input, flip_input, 2)
            feature_fc = (feat_fc.detach() + feat_fc_1.detach()) / 2
            fnorm_fc = torch.norm(feature_fc, p=2, dim=1, keepdim=True)
            feature_fc = feature_fc.div(fnorm_fc.expand_as(feature_fc))
...
def main():
    ...
    log_s1_name = 'sysu_2p_384_g_s12'

    log_s2_name = 'sysu_2p_288_g_s2'
...
def main_worker_stage2(args, log_s1_name, log_s2_name):
    ...
    for epoch in range(args.epochs):
        if epoch > 100:
            ...
            for rgb_cam_id in sorted(rgb_cams):
                ...
                for ir_cam_id in sorted(ir_cams):
                    ...
                    features_rgb_dict,_, _ = extract_features(model, cluster_loader_rgb, print_freq=50, mode=1)
                    ...
                    print('==> Statistics for RGB  epoch {}: camera {} {} clusters,cluster data {} '.format(epoch,
                                                                                                            rgb_cam_id,
                                                                                                            num_cluster_rgb,
                                                                                                            len(pseudo_labeled_dataset_rgb)))
                    ...
                    features_ir_dict,_, _ = extract_features(model, cluster_loader_ir, print_freq=50, mode=2)
                    ...
                    print('==> Statistics for RGB  epoch {}: camera {} {} clusters,cluster data {} '.format(epoch,
                                                                                                            ...
                                                                                                            num_cluster_rgb,
                                                                                                            len(pseudo_labeled_dataset_rgb)))
                    print('==> Statistics for ir  epoch {}: camera {} {} clusters,cluster data {} '.format(epoch,
                                                                                                           ir_cam_id,
                                                                                                           num_cluster_ir,
                                                                                                           len(pseudo_labeled_dataset_ir)))
                    ...
                    time.sleep(1)
                    trainer_intrac.train(epoch, train_loader_ir, train_loader_rgb, optimizer, print_freq=10,
                                         train_iters=len(train_loader_rgb))
        ...
        with torch.no_grad():
            ...
            features_rgb, features_rgb2ir, _ = extract_features(model, cluster_loader_rgb, print_freq=50, mode=1)

            del cluster_loader_rgb
            features_rgb = torch.cat([features_rgb[f].unsqueeze(0) for f, _, _ in sorted(dataset_rgb.train)], 0)
            features_rgb_ = F.normalize(features_rgb, dim=1)

            features_rgb2ir = torch.cat([features_rgb2ir[f].unsqueeze(0) for f, _, _ in sorted(dataset_rgb.train)], 0)
            features_rgb2ir_ = F.normalize(features_rgb2ir, dim=1)
            ...
            features_ir, features_ir2rgb, _ = extract_features(model, cluster_loader_ir, print_freq=50, mode=2)

            del cluster_loader_ir
            features_ir = torch.cat([features_ir[f].unsqueeze(0) for f, _, _ in sorted(dataset_ir.train)], 0)
            features_ir_ = F.normalize(features_ir, dim=1)

            features_ir2rgb = torch.cat([features_ir2rgb[f].unsqueeze(0) for f, _, _ in sorted(dataset_ir.train)], 0)
            features_ir2rgb_ = F.normalize(features_ir2rgb, dim=1)
            ...
            rerank_dist_ir = compute_jaccard_distance(features_ir_, k1=args.k1, k2=args.k2,
                                                      search_option=3)  # rerank_dist_all_jacard[features_rgb.size(0):,features_rgb.size(0):]#
            # rerank_dist_ir2rgb = compute_jaccard_distance(features_ir2rgb_, k1=args.k1, k2=args.k2,
            #                                          search_option=3)  # rerank_dist_all_jacard[features_rgb.size(0):,features_rgb.size(0):]#
            pseudo_labels_ir = cluster_ir.fit_predict(rerank_dist_ir)
            # pseudo_labels_ir2rgb = cluster_ir.fit_predict(rerank_dist_ir2rgb)

            rerank_dist_rgb = compute_jaccard_distance(features_rgb_, k1=args.k1, k2=args.k2,
                                                       search_option=3)  # rerank_dist_all_jacard[:features_rgb.size(0),:features_rgb.size(0)]#
            pseudo_labels_rgb = cluster_rgb.fit_predict(rerank_dist_rgb)

            # rerank_dist_rgb2ir = compute_jaccard_distance(features_rgb2ir_, k1=args.k1, k2=args.k2,
            #                                           search_option=3)  # rerank_dist_all_jacard[:features_rgb.size(0),:features_rgb.size(0)]#
            # pseudo_labels_rgb2ir = cluster_rgb.fit_predict(rerank_dist_rgb2ir)

            del rerank_dist_rgb
            del rerank_dist_ir
            # del rerank_dist_rgb2ir
            # del rerank_dist_ir2rgb

            pseudo_labels_all = []
            num_cluster_ir = len(set(pseudo_labels_ir)) - (1 if -1 in pseudo_labels_ir else 0)
            num_cluster_rgb = len(set(pseudo_labels_rgb)) - (1 if -1 in pseudo_labels_rgb else 0)

            # pseudo_labels_all_trans = []
            # num_cluster_ir2rgb = len(set(pseudo_labels_ir2rgb)) - (1 if -1 in pseudo_labels_ir2rgb else 0)
            # num_cluster_rgb2ir = len(set(pseudo_labels_rgb2ir)) - (1 if -1 in pseudo_labels_rgb2ir else 0)
        ...
        cluster_features_ir2rgb = generate_cluster_features(pseudo_labels_ir, features_ir2rgb)
        cluster_features_rgb2ir = generate_cluster_features(pseudo_labels_rgb, features_rgb2ir)
        ...
        memory_ir2rgb = ClusterMemory(model.module.in_planes * part, num_cluster_ir, temp=args.temp,
                                      momentum=args.momentum, use_hard=args.use_hard).cuda()
        memory_rgb2ir = ClusterMemory(model.module.in_planes * part, num_cluster_rgb, temp=args.temp,
                                      momentum=args.momentum, use_hard=args.use_hard).cuda()

        memory_ir.features = F.normalize(cluster_features_ir, dim=1).cuda()
        memory_rgb.features = F.normalize(cluster_features_rgb, dim=1).cuda()

        memory_ir2rgb.features = F.normalize(cluster_features_ir2rgb, dim=1).cuda()
        memory_rgb2ir.features = F.normalize(cluster_features_rgb2ir, dim=1).cuda()

        trainer.memory_ir = memory_ir
        trainer.memory_rgb = memory_rgb

        trainer.memory_ir2rgb = memory_ir2rgb
        trainer.memory_rgb2ir = memory_rgb2ir
        ...
        if epoch >= trainer.cmlabel:
            ...
            ins_label_rgb_ir = torch.from_numpy(pseudo_labels_ir)[
                ins_indices_rgb_ir_2].detach().cpu()  # .numpy()#.view(-1)
            cluster_label_rgb_ir = ins_label_rgb_ir  # ins_label_rgb_ir_2#ins_label_rgb_ir_2

            for l in range(20):
                intersect_count = (ins_label_rgb_ir == cluster_label_rgb_ir[:, l].view(-1, 1)).int().sum(1).view(-1,
                                                                                                                 1).detach().cpu()
            ...
            cluster_label_rgb_ir = torch.gather(cluster_label_rgb_ir, dim=1, index=cluster_label_index.view(-1,
                                                                                                            1))  # cluster_label_rgb_ir[cluster_label_index.reshape(-1,1)]
            ...
            for i in range(int(lenth_ratio * len(rgb2ir_label_cnt_sorted))):
                ...
                if key[0] == key[1]:
                    continue
                print('rgb-ir label{}, real rgb label{}, real ir label{}'.format(key, pseudo_real_rgb[key[0]],
                                                                                 pseudo_real_ir[key[1]]))
            ...
            pseudo_labels_all = torch.cat((torch.from_numpy(pseudo_labels_rgb), torch.from_numpy(pseudo_labels_ir)),
        ...
        if epoch >= trainer.cmlabel:
            trainer.train2(epoch, train_loader_ir, train_loader_rgb, optimizer, all_label=pseudo_labels_all,
                               intra_id_labels_all=intra_id_labels_all, cams_all=modality_rgb,
                               intra_id_features_all=intra_id_features_all,
                               intra_id_labels_rgb=intra_id_labels_rgb, intra_id_features_rgb=intra_id_features_rgb,
                               intra_id_labels_ir=intra_id_labels_ir, intra_id_features_ir=intra_id_features_ir,
                               all_label_rgb=pseudo_labels_rgb, all_label_ir=pseudo_labels_ir, cams_ir=cams_ir,
                               cams_rgb=cams_rgb, cross_cam=cross_cam,
                               intra_id_features_crosscam=intra_id_features_ccam,
                               intra_id_labels_crosscam=intra_id_labels_ccam,
                               print_freq=args.print_freq, train_iters=len(train_loader_ir))
        else:
            trainer.train(epoch, train_loader_ir, train_loader_rgb, optimizer, all_label=pseudo_labels_all,
                          intra_id_labels_all=intra_id_labels_all, cams_all=modality_rgb,
                          intra_id_features_all=intra_id_features_all,
                          intra_id_labels_rgb=intra_id_labels_rgb, intra_id_features_rgb=intra_id_features_rgb,
                          intra_id_labels_ir=intra_id_labels_ir, intra_id_features_ir=intra_id_features_ir,
                          all_label_rgb=pseudo_labels_rgb, all_label_ir=pseudo_labels_ir, cams_ir=cams_ir,
                          cams_rgb=cams_rgb, cross_cam=cross_cam, intra_id_features_crosscam=intra_id_features_ccam,
                          intra_id_labels_crosscam=intra_id_labels_ccam,
                          print_freq=args.print_freq, train_iters=len(train_loader_ir))
        ...
        if epoch >= 0 and ((epoch + 1) % args.eval_step == 0 or (epoch == args.epochs - 1)):
            ...
            for trial in range(10):
                ...
                print('Test Trial: {}'.format(trial))
                print(
                    'indoor FC:   Rank-1: {:.2%} | Rank-5: {:.2%} | Rank-10: {:.2%}| Rank-20: {:.2%}| mAP: {:.2%}| mINP: {:.2%}'.format(
                        cmc[0], cmc[4], cmc[9], cmc[19], mAP, mINP))
